...
module.exports = functions.region('asia-northeast1').https.onRequest(async (request, response) => {
    ...
    if (-1 < notification.openpersons.indexOf(personuid)) {
        console.log('スルー4 多重開封はあり得るので良しとする');
        response.status(200).end(JSON.stringify({ data: true }));
        return;
    }

    await firestore.collection('notifications').doc(notificationid).update({ openpersons: admin.firestore.FieldValue.arrayUnion(personuid) });
